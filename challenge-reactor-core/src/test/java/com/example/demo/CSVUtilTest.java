...
public class CSVUtilTest {
    ...
    @Test
    void reactive_filtrarJugadoresMayoresA34Equipo(){
        List<Player> list = CsvUtilFile.getPlayers();
        Flux<Player> listFlux = Flux.fromStream(list.parallelStream()).cache();
        Mono<Map<String, Collection<Player>>> listFilter = listFlux
                .filter(player -> player.age >= 34 && player.club.equals("SL Benfica"))
                .distinct()
                .collectMultimap(Player::getClub);
        System.out.println("En el Equipo: ");
        listFilter.block().forEach((equipo, players) -> {
            ...
            players.forEach(player -> {
                ...
                assert player.club.equals("SL Benfica");
            });
        });
